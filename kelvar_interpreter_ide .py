...
class RuntimeVariable:
    def __init__(self, name: str, val_type: str, value, is_mutable: bool = True):
        self.name = name
        self.type = val_type        # 'int', 'string', 'pointer', 'reference', 'function', 'list', 'tuple', 'struct_def', 'struct_instance', 'class_def', 'class_instance', 'builtin', 'generator', 'float', 'bool', 'char', 'datetime'
        self.value = value          
        self.is_active = True       
        self.moved_to = None        
        self.is_mutable = is_mutable 
            ...
            if callee_type == "function":
                ...
                while curr_class:
                    for method in curr_class.methods:
                        if method.name not in resolved_methods:
                            resolved_methods[method.name] = method
    ...
    def execute(self, node: ASTNode, env: Environment):
        if isinstance(node, LetNode):
            ...
            if isinstance(node.expr, VarAccessNode):
                ...
                if src and src.type in ["pointer", "reference", "struct_instance", "class_instance", "list", "tuple", "function", "generator"]:
        ...
        if isinstance(node, FnNode):
            ...
            self.log_success(f"Saved custom system logic framework routing module label setup structural path blueprint: fn {node.name}()")
            return
...
class KelvarIDE:
    ...
    def build_layout(self):
        ...
        ttk.Label(header, text="🛡️ KELVAR COMPILER LABS IDE", font=("Consolas", 14, "bold"), foreground="#2ed573").pack(side=tk.LEFT)
        ...
        demo_frame.pack(side=tk.RIGHT)
        
        # Theme Selector Dropdown
        ttk.Label(demo_frame, text="Theme: ").pack(side=tk.LEFT, padx=2)
        self.theme_selector = ttk.Combobox(demo_frame, values=list(self.themes.keys()), width=15, state="readonly")
        ...
        ttk.Label(demo_frame, text="Load Framework Demo: ").pack(side=tk.LEFT, padx=5)
        self.demo_selector = ttk.Combobox(demo_frame, values=["Mutable & Loops", "For-In Loops", "Comprehension & Dynamic Typing", "Rich Types & DateTime", "Function Recursion", "Rust Move Safety", "C Pointer Arithmetic", "Structs, OOP & Collections", "Graphics Drawing", "Slicing, Functions & Generators"], width=28, state="readonly")
        self.demo_selector.pack(side=tk.LEFT, padx=5)
        self.demo_selector.bind("<<ComboboxSelected>>", self.load_selected_demo)
        ...
        btn_bar = ttk.Frame(editor_panel)
        btn_bar.pack(fill=tk.X, pady=5)
        
        tk.Button(btn_bar, text="▶ Run Execution Loop", font=("Consolas", 11, "bold"), bg="#007acc", fg="white", activebackground="#0097e6", relief=tk.FLAT, bd=0, padx=15, pady=5, command=self.run_compiled_code).pack(side=tk.LEFT, padx=5)
        
        # File System action bindings for .kv operations
        tk.Button(btn_bar, text="📁 Open File (.kv)", font=("Consolas", 10), bg="#2f3542", fg="white", relief=tk.FLAT, bd=0, padx=10, pady=5, command=self.open_file).pack(side=tk.LEFT, padx=5)
        tk.Button(btn_bar, text="💾 Save File (.kv)", font=("Consolas", 10), bg="#2f3542", fg="white", relief=tk.FLAT, bd=0, padx=10, pady=5, command=self.save_file).pack(side=tk.LEFT, padx=5)
        
        tk.Button(btn_bar, text="🧹 Wipe Editor", font=("Consolas", 10), bg="#2f3542", fg="white", relief=tk.FLAT, bd=0, padx=10, pady=5, command=self.clear_editor).pack(side=tk.LEFT, padx=5)
        ...
        self.editor.insert(tk.END, "// Welcome to Kelvar Revolution Pro Sandbox Engine!\n// Choose the 'Structs, OOP & Collections' or 'Rich Types & DateTime' demo above.\n\nlet mut asset_matrix = [100, 200, 300]\nprint asset_matrix\n")
        self.highlight_syntax()

    def change_theme(self, event=None):
        self.current_theme = self.themes[self.theme_selector.get()]
        self.apply_theme()
        self.highlight_syntax()
    ...
    def run_compiled_code(self):
        ...
        # Inject Canvas Widget context right into Interpreter mapping flow
        ...
        except KelvarSecurityError as kse: self.log_to_gui_console(str(kse), "security")
        except (SyntaxError, TypeError, NameError, IndexError, AttributeError, RuntimeError) as err: self.log_to_gui_console(f"Fault Trace Intersection: {err}", "security")
        except Exception as e: self.log_to_gui_console(f"Unmapped runtime breakdown trace: {e}", "security")
    ...
    def load_selected_demo(self, event=None):
        selected = self.demo_selector.get()
        self.clear_editor()
